...
from backend.core.services.video_cache_service import get_video_hash, get_cached_video_session, save_video_session
...
def _run_pipeline_in_thread(job_id: str, video_path: str, session_id: str, width: int, height: int):
    ...
    try:
        ...
        def _update_progress(pct, line_counts=None, active_lane=None, lane_state=None):
            job_store[job_id]["progress"] = pct
            if line_counts is not None:
                job_store[job_id]["live_crossings"] = line_counts
            if active_lane is not None:
                job_store[job_id]["active_lane"] = active_lane
            if lane_state is not None:
                # Extract just the vehicle counts per lane for the frontend
                job_store[job_id]["live_lane_counts"] = {
                    lane: int(data.get("count", 0))
        ...
        # Update session status to completed in DB
        try:
            conn = get_connection()
            cursor = conn.cursor()
            cursor.execute("UPDATE simulation_session SET status = 'completed' WHERE id = ?", (session_id,))
            conn.commit()
            conn.close()
        except Exception as e:
            print(f"⚠️ [JOB] Failed to update session status: {e}")

        # Save mapping to cache database
        try:
            video_hash = get_video_hash(video_path)
            save_video_session(video_hash, session_id)
            print(f"💾 [CACHE] Saved video session mapping: hash={video_hash} -> session={session_id}")
        except Exception as e:
            print(f"⚠️ [CACHE] Failed to save mapping: {e}")
...
async def run_video_pipeline_job(job_id: str, video_path: str):
    ...
    try:
        video_hash = get_video_hash(video_path)
        cached_session_id = get_cached_video_session(video_hash)
        
        if cached_session_id:
            # Verify that the session is complete and has results
            conn = get_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT status, timer_duration FROM simulation_session WHERE id = ?", (cached_session_id,))
            s_row = cursor.fetchone()
            cursor.execute("SELECT COUNT(*) FROM simulation_result WHERE session_id = ?", (cached_session_id,))
            r_count = cursor.fetchone()[0]
            conn.close()

            if s_row and s_row[0] == "completed" and r_count > 0:
                timer_duration = s_row[1]
                print(f"📦 [CACHE] Video matches completed session={cached_session_id}. Instantly loading results.")
                
                # Instantly mark job as completed in job_store
                job_store[job_id]["status"] = "completed"
                job_store[job_id]["progress"] = 100
                job_store[job_id]["session_id"] = cached_session_id
                job_store[job_id]["timer_duration"] = timer_duration
                job_store[job_id]["video_duration"] = timer_duration
                job_store[job_id]["video_events"] = []
                return
