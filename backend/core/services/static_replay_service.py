...
def run_static_replay_simulation(events, timer_duration):
    ...
    return {
        ...
        'total_green_ticks': total_green_ticks,
        'utilized_green_ticks': utilized_green_ticks
    }
...
def compute_dynamic_metrics(events, timer_duration=None):
    ...
    total_green_ticks = replay.get('total_green_ticks', 0)
    utilized_green_ticks = replay.get('utilized_green_ticks', 0)
    avg_green_utilization = round((utilized_green_ticks / total_green_ticks * 100.0), 2) if total_green_ticks > 0 else 0.0
    
    if not all_records:
        return {'avg_wait_time': 0, 'total_vehicles_crossed': 0, 'co2_estimate': 0, 'avg_green_utilization': avg_green_utilization, 'ambulance_avg_wait_time': 0}

    # Filter to only include vehicles that successfully crossed to avoid censoring bias
    crossed_records = [r for r in all_records if r.get('crossedAt') is not None]
    
    if not crossed_records:
        return {
            'avg_wait_time': 0.0,
    ...
    waits = [r['waitTime'] for r in crossed_records]
    avg_wait = compute_avg_wait_time(waits)
    
    amb_waits = [
        r['waitTime'] for r in all_records 
        if 'v2i' in str(r['vehicleId']).lower() or 'ambulance' in str(r['vehicleType']).lower()
    ]

    return {
        'avg_wait_time': avg_wait,
        'total_vehicles_crossed': len(crossed_records),
