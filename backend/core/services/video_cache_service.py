...
def get_cached_video_session(video_hash):
    """Checks if we have already processed this video and returned a successful session."""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT session_id FROM video_session_map WHERE video_hash = ?", (video_hash,))
    row = cursor.fetchone()
    conn.close()
    if row:
        return row[0]
...
def save_video_session(video_hash, session_id):
    """Saves a mapping from video hash to session_id."""
    from datetime import datetime
    conn = get_connection()
    cursor = conn.cursor()
    created_at = datetime.utcnow().isoformat()
    cursor.execute("INSERT OR REPLACE INTO video_session_map (video_hash, session_id, created_at) VALUES (?, ?, ?)", (video_hash, session_id, created_at))
    conn.commit()
    conn.close()
