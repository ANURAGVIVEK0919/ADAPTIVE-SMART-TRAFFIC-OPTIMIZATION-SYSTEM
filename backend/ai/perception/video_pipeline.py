...
from backend.core.services.video_cache_service import (
    get_video_hash, 
...
LANE_ORDER = ['north', 'east', 'south', 'west'] # 🔄 Cyclic order aligned with frontend
...
def post_frame_async(img_bytes, base_url):
    global _last_post_time
    now = time.time()
    if now - _last_post_time < 0.15:  # Limit to ~6-7 fps to keep it lightweight
        return
    _last_post_time = now
    
    def post():
        try:
            requests.post(f"{base_url}/simulation/video-frame", data=img_bytes, timeout=0.2)
        except Exception:
            pass
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                if resp and 'action' in resp:
                    with shared_lock:
                        # 3. Prevent Race Condition: verify that active_lane hasn't changed during prediction
                        current_active_lane = shared_state['active_green_lane']
                        if current_active_lane != active_lane:
                            if DEBUG:
                                print(f"⚠️ [RL WORKER] Stale decision ignored (was for {active_lane}, now {current_active_lane})")
                        else:
                            # Enforce Cyclic Logic: Switch means go to the NEXT lane in cycle
    ...
    try:
        while True:
            ...
            if False and cached_data and frame_index in cache_map: # Disabled to force fresh metric recalculation
                ...
                last_lane_state = {
                    lane: {
                        'count': c['lane_counts'].get(lane, 0),
                        'hasAmbulance': c['ambulances'].get(lane, False),
                        'avgWaitTime': c['wait_times'].get(lane, 0.0)
                    }
                    for lane in TRAFFIC_DIRECTIONS
                }
                # Sync other variables for downstream logic
                line_counts = {lane: 0 for lane in TRAFFIC_DIRECTIONS}
                wait_time_by_direction = c['wait_times']
                queue_length_by_direction = c['lane_counts']
            elif live_detection_seq != last_processed_detection_seq:
                ...
                # SAVE TO CACHE (only if not already cached)
                if not cached_data and frame_index % 5 == 0: # Save every 5th frame to save space
                    save_frame_to_cache(
                        video_hash, 
                        frame_index, 
                        {l: last_lane_state[l]['count'] for l in TRAFFIC_DIRECTIONS},
                        {l: last_lane_state[l]['hasAmbulance'] for l in TRAFFIC_DIRECTIONS},
                        wait_time_by_direction
                    )
            ...
            for track in tracks:
                ...
                if lane not in TRAFFIC_DIRECTIONS:
                    continue
                center = track.get('center') or track.get('centroid') or (0, 0)
            ...
            from backend.infra.database.shared_state import set_lane_counts
                ...
                try:
                    ...
                    if v2i_resp.status_code == 200:
                        ...
                        for beacon in active_beacons:
                            ...
                            b_lane = _normalize_direction_label(beacon.get('lane'))
                            if b_lane in v2i_ambulances:
                                v2i_ambulances[b_lane] = True
                                # Add as virtual vehicle for metrics if new
                                if vid not in v2i_seen_ids:
                                    v2i_seen_ids.add(vid)
                                    v2i_active_ids.add(vid)
                                    events.append({
                        ...
                        removed_v2i = v2i_active_ids - current_v2i_ids
                        for vid in removed_v2i:
                            events.append({
                                "eventType": "vehicle_crossed",
                                "vehicleId": vid,
                                "timestamp": run_start_ms + int(current_time * 1000)
                            })
                            v2i_active_ids.remove(vid)
                            
                except Exception as e:
                    if DEBUG: print(f"⚠️ [V2I POLL] Failed to fetch V2I status: {e}")

            # Merge V2I alerts into counts and lane state
            for lane in TRAFFIC_DIRECTIONS:
                if v2i_ambulances[lane]:
                    # Ensure they are counted even if not visually detected
                    if current_counts.get(lane, 0) == 0:
                        current_counts[lane] = 1
                    
                    if lane in last_lane_state:
                        last_lane_state[lane]['hasAmbulance'] = True
                        last_lane_state[lane]['count'] = max(last_lane_state[lane]['count'], 1)
                    if lane in smoothed_lane_state:
                        smoothed_lane_state[lane]['hasAmbulance'] = True
                        smoothed_lane_state[lane]['count'] = max(smoothed_lane_state[lane]['count'], 1)
            ...
            if len(events) >= 20 or (current_time_wall - last_flush_time > 2):
                ...
                lane_state_snapshot = {
                    lane: {
                        "count": int(current_counts.get(lane, 0)),
                        "hasAmbulance": v2i_ambulances[lane] or last_lane_state.get(lane, {}).get('hasAmbulance', False)
                    }
                    for lane in TRAFFIC_DIRECTIONS
                }
                ...
                events.append({
                    ...
                    "payload": {
                        "snapshot": {
                            ...
                            "active_lane": active_green_lane or "north",
                            "duration": 10
                        }
                    }
                })
            ...
            if sample_due:
                ...
                if next_green_lane != active_green_lane:
                    ...
                    with shared_lock:
                        ...
                        shared_state['latest_signal_phases'] = list(signal_phases)
                        shared_state['latest_decision'] = {'lane': active_green_lane, 'duration': 10}
                ...
                if on_progress:
                    ...
                    if pct > _last_progress_report:
                        on_progress(
                            pct,
                            line_counts=dict(shared_state.get('latest_line_counts', {})),
                            active_lane=shared_state.get('active_green_lane', None),
                            lane_state=dict(shared_state.get('latest_lane_state', {})),
                        )
                        _last_progress_report = pct

                # Send clean frame to frontend preview (no bounding boxes/debug overlays)
                clean_frame = frame if frame is not None else last_overlay
                if clean_frame is not None:
                    try:
                        ok, encoded_img = cv2.imencode('.jpg', clean_frame)
                        if ok:
                            post_frame_async(encoded_img.tobytes(), base_url)
                    except Exception as e:
                        if DEBUG: print(f"⚠️ [LIVE PREVIEW] imencode/post failed: {e}")

                # Local writer and preview use the debug overlay (with boxes)
                overlay_for_local = last_overlay if last_overlay is not None else frame
                if writer is not None:
                    writer.write(overlay_for_local)
                if show_preview:
                    cv2.imshow('Traffic RL Output', overlay_for_local)
